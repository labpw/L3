...
namespace MVCClient.Repositories
{
    public class CarBrandRepository: ICarBrandRepository
    {
        ...
        public CarBrandRepository(HttpClient httpClient, IOptions<AppSettings> appSettings)
        {
        ...
        public async Task<ServiceResponse> CreateCarBrandAsync(CarBrand carBrand)
        {
            ...
            HttpContent contentToUpdate = new StringContent(
                json,
                Encoding.UTF8,
                "application/json"
            );
            var response = await httpClient.PostAsync(
                string.Format(appSettings.CarBrandsEndpoint.Create, carBrand.Id),
                contentToUpdate
            );
        ...
        public async Task<ServiceResponse> DeleteCarBrandAsync(int carBrandId)
        {
            var response = await httpClient.DeleteAsync(
                string.Format(appSettings.CarBrandsEndpoint.Delete, carBrandId)
            );
            var result = new ServiceResponse
            {
                Message = "Car Brands retrieved successfully.",
        ...
        public async Task<ServiceResponse> UpdateCarBrandAsync(CarBrand carBrand)
        {
            ...
            var response = await httpClient.PutAsync(
                string.Format(appSettings.CarBrandsEndpoint.Update, carBrand.Id),
                contentToUpdate
            );
            var result = new ServiceResponse
            {
